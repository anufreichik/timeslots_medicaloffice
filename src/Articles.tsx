...
/**
 * Необходимо разработать приложение для заметок. Основной экран - список заметок. В
 нём есть поле поиска заметок. При вводе текста в это поле должна происходить фильтрация
 заметок по наличию искомого текста в названии или тексте заметки. Возможно создавать и
 удалять заметки. У заметок только два поля - название и текст. Название - опциональное.
 Новая заметка добавляется в начало списка.
 Технический стек:
 Все данные хранятся в local storage.
 UX придумать самостоятельно.
 Что мы оцениваем и проверяем:
 - качество кода
 - архитектурный подход и разделение приложения на корректные слои
 - наличие ошибок в логике
 - инструкцию по поставке и легкость развертывания приложения по ней
 * @constructor
 */
